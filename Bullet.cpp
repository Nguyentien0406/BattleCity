...
void Bullet::update(vector<Wall>& walls, PlayerTank& player, PlayerTank& otherPlayer, vector<EnemyTank>& enemies, Game& game) {
    switch(direction) {
    ...
    if(player.isAlive()) {
        ...
        if(!isFromPlayer && SDL_HasIntersection(&rect, &playerRect)) {
            ...
            if(player.isDestroyed()) {
                ...
                game.addExplosion(player.getRect().x, player.getRect().y);
    ...
    if(otherPlayer.isAlive()) {
        ...
        if(!isFromPlayer && SDL_HasIntersection(&rect, &otherPlayerRect)) {
            ...
            if(otherPlayer.isDestroyed()) {
                ...
                game.addExplosion(otherPlayer.getRect().x, otherPlayer.getRect().y);
                game.playExplosionSound();
                game.setGameOver(player, otherPlayer);
            }
            collided= true;
